...
## 1. Safety Directives & the `agent-crash.sh` Wrapper
...
**Mandate:** Always use the provided `agent-crash.sh` wrapper script instead of calling `crash` directly.
```bash
# General Syntax
./agent-crash.sh -k /path/to/vmlinux -c /path/to/vmcore <macro_or_command>

# Example: Fallback raw command execution (Safe, truncated, non-blocking)
./agent-crash.sh -k vmlinux -c vmcore run "rd ffff880123456789 128"
```
...
- If `agent-crash.sh triage` reveals a `kernel BUG at fs/pipe.c:120!`, the Agent should parse the signature and perform a quick web search or `git grep` on the upstream Linux kernel to see if this has already been fixed. Do not reinvent the wheel if a known CVE/Bugzilla is present.
...
- **Command**: `agent-crash.sh -k vmlinux dis-regs <faulty_function> <panicked_pid>`
...
- **Command**: `agent-crash.sh -k vmlinux check-poison <suspicious_address>`
...
- Use `./agent-crash.sh -k vmlinux run "<command>"` for primitives.
